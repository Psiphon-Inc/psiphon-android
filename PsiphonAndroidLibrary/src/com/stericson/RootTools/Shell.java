...
public class Shell {
    ...
    private Shell(String cmd) throws IOException, TimeoutException {
        ...
        try
        {
            // PSIPHON: allow more time for Superuser prompt; we still retry, but clicking
            // the first prompt instance guarantees no duplicate prompt (race conditon).
            // The tradeoff is this longer timeout will cause a longer delay in the case
            // of the Runtime.getRuntime.exec() hang on Android 4.0.x. This also causes
            // a longer delay before the prompt comes back when the user clicks the Home
            // button, for example.
        	worker.join(10000);
        	
            if (worker.exit == Worker.TIMEOUT_ERROR)
            {
            	proc.destroy();
            ...
            if (worker.exit == Worker.OTHER_ERROR)
            {
                proc.destroy();
		...
		if (rootShell == null) {
			...
			while (rootShell == null) {
				try {
				    ...
				    // PSIPHON: no retries, to avoid re-prompting on deny.
					//if (retries++ >= 2)
					{
    ...
    {
        // PSIPHON: properly distinguish between timeout and other conditions

        public static int TIMEOUT_ERROR = -1;
        public static int OTHER_ERROR = 0;
        public static int NO_ERROR = 1;
        
    	public int exit = TIMEOUT_ERROR;
