...
public class AppUpdatePolicy {
    ...
    public static final String PREF_SERVER_UPDATE_POLICY = "server_update_policy";
    public static final String PREF_UPDATE_POLICY_TIMESTAMP_MS = "server_update_policy_timestamp_ms";
    ...
    public boolean shouldForceUpdate(int currentVersion, int playPriority, int stalenessDays) {
        ...
        if (mustUpdateVersions.contains(currentVersion)) return true;
        if (cutoffVersion > 0 && currentVersion < cutoffVersion) return true;
        if (playPriority >= PLAY_IMMEDIATE_THRESHOLD) return true;
